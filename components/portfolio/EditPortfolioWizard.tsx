...
export interface EditPortfolioInitial {
  id:            string
  name:          string
...
type WizardState = {
  name:          string
  file:          File | null         // new file if user uploaded one
  positions:     ParsedPosition[]    // parsed from new file (empty = keep existing)
  parseError:    string
  riskTolerance: RiskTolerance | ''
  horizon:       Horizon | ''
  objective:     Objective | ''
...
export default function EditPortfolioWizard({ initial }: { initial: EditPortfolioInitial }) {
  ...
  // Positions to preview: new ones if file uploaded, otherwise existing
  const previewPositions: { ticker: string; quantity: number; avgBuyPrice: number }[] =
    state.positions.length > 0 ? state.positions : initial.positions
  ...
  // ── Step 2 → 3: parse if new file, otherwise go directly ──
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Only send positions if user uploaded a new file
      if (state.positions.length > 0) {
        body.positions = state.positions
      }

      const res  = await fetch(`/api/portfolios/${initial.id}`, {
        method:  'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify(body),
      })
      const data = await res.json()
      if (!res.ok) { setSubmitError(data.error ?? 'Error al guardar los cambios.'); return }
      router.push(`/portfolio/${initial.id}`)
    } catch {
  ...
  const step1Valid = state.name.trim().length >= 2
  ...
  return (
    <div style={{ maxWidth: '580px', margin: '0 auto' }}>
      ...
      {step === 1 && (
        <div>
          <h1 className="t-h2" style={{ color: 'var(--text-1)', marginBottom: '6px' }}>Editá la cartera</h1>
          <p className="t-body" style={{ color: 'var(--text-2)', marginBottom: '32px' }}>
            Modificá el nombre o subí un nuevo archivo de posiciones.
          </p>
          ...
          >
            ...
            {state.file ? (
              ...
              <>
                <i className="fa-solid fa-file-lines" style={{ fontSize: '28px', color: 'var(--green-core)', marginBottom: '8px' }} />
                <p className="t-body" style={{ color: 'var(--text-1)', fontWeight: 500 }}>
                  {initial.positions.length} posiciones cargadas
                </p>
                <p className="t-small" style={{ color: 'var(--text-3)' }}>Arrastrá o hacé clic para reemplazar el archivo</p>
              </>
            )}
          </div>
      ...
      {step === 2 && (
        <div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <button className="btn btn-filled" disabled={!step2Valid} onClick={goToPreview}>
      ...
      {step === 3 && (
        <div>
          <h1 className="t-h2" style={{ color: 'var(--text-1)', marginBottom: '6px' }}>Confirmá los cambios</h1>
          <p className="t-body" style={{ color: 'var(--text-2)', marginBottom: '32px' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              ...
              <div>
                ...
                <p className="t-small" style={{ color: 'var(--text-1)' }}>
                  {previewPositions.length} activos
                  {state.positions.length > 0 && (
                    <span style={{ color: 'var(--green-core)', marginLeft: '6px' }}>· nuevo archivo</span>
                  )}
                </p>
              </div>
            </div>
